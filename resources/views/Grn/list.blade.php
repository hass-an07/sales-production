...
@section('content')
        ...
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{ route('grn.create') }}" class="btn btn-primary">Add Grn</a>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->
    ...
    <section class="content">
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <form action="{{ route('grn.index') }}" method="GET">
                    <div class="card-header">
                        @include('mesage')
                        <div class="card-title">
                            <button type="button" onclick="window.location.href='{{ route('grn.index') }}'"
                            ...
                            @if ($grns->isNotEmpty())
                                @foreach ($grns as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->po_order_no ?? 'N/A' }}</td>                                       
                                        <td>{{ $item->invoice_no ?? 'N/A' }}</td>                                       
                                        <td>{{ $item->total_amount ?? 'N/A' }}</td>                                       
                                        <td class="d-flex">
                                            <!-- Edit Button -->
                                            {{-- <a href="{{ route('grn.edit', $item->id) }}" class="mr-2">
                                            ...
                                            <!-- Delete Button -->
                                            <form action="{{ route('grn.destroy', $item->id) }}" method="POST"
