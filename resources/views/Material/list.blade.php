        ...
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                {{-- <div class="col-sm-6 text-right">
                    <a href="{{ route('materialType.create') }}" class="btn btn-primary">Add Material</a>
                </div> --}}
            </div>
        </div>
        <!-- /.container-fluid -->
        ...
        <div class="card">
            <form action="{{ route('material.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="card-header ">
                    ...
                    <div class="card-tools py-4 col-12">
                    ...
                    <tbody>
                        @if ($material->isNotEmpty())
                            @foreach ($material as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>{{ $item->materialType->material_type }}</td>
                                    <td>{{ $item->material }}</td>
                                    <td>{{ $item->unit }}</td>
                                    <td class="d-flex">
                                        <a href="{{ route('material.edit',$item->id) }}">
                                         ...
                                         <!-- Delete Button -->
                                         <form action="{{ route('material.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                            @csrf
                                            @method('DELETE')
